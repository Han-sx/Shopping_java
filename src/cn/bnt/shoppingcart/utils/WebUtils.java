...
public class WebUtils {
	...
	public static <T> T request2Bean(HttpServletRequest request,Class<T> clazz)  {
		...
		try {
			...
			t=clazz.newInstance();//由clazz二进制字节码对象来创建对应的对象类型
	...
	public static String getOrderOid() {//产生订单编号
		String order="";
		order=System.currentTimeMillis()+(int)Math.random()*10000+"";
		order=order.substring(order.length()-5);
		return order;
	}
	
	public static Map validate(HttpServletRequest request) {
